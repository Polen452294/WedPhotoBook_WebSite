...
const PUBLIC_CONTENT_SECURITY_POLICY = [
  ...
  "object-src 'none'",
  `script-src 'self' 'unsafe-inline'${DEVELOPMENT_EVAL_SOURCE} ${YANDEX_METRIKA_SOURCES} https://yastatic.net ${TALK_ME_SOURCES}`,
  `style-src 'self' 'unsafe-inline' ${TALK_ME_SOURCES}`,
  `img-src 'self' data: blob: https://*.yandex.ru https://*.yandex.net ${YANDEX_METRIKA_SOURCES} ${TALK_ME_SOURCES}`,
  `font-src 'self' data: ${TALK_ME_SOURCES}`,
  `connect-src 'self' https://*.yandex.ru https://*.yandex.net ${YANDEX_METRIKA_CONNECTIONS} ${TALK_ME_CONNECTIONS}`,
