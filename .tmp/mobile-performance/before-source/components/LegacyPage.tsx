...
import { isPhotoSource, optimizedMediaUrl, photoMediaUrl } from "@/lib/media-path";
...
function withResponsiveLegacyImages(bodyHtml: string, slug: string): string {
  ...
  return bodyHtml.replace(/<img\b[^>]*>/gi, (tag) => {
    ...
    if (src && isPhotoSource(src)) {
      // Captured galleries may point at thumbnails; keep their layout dimensions
      // while serving the complete original, including on the local dev server.
      return tag
        .replace(/\bsrc=("|')([^"']+)\1/i, `src="${photoMediaUrl(src).replaceAll("&", "&amp;")}"`)
...
export function LegacyPage({ page }: { page: RenderedPage }) {
  ...
  return (
    <>
      <link
        rel="stylesheet"
        href={isHomepage ? "/wp-assets/home-optimized.css?v=3" : "/wp-assets/wordpress.css?v=11"}
