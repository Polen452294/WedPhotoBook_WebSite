...
- `RESEND_API_KEY`, `CONTACT_TO_EMAIL`, `CONTACT_FROM_EMAIL` — обязательны для доставки уведомлений о новых заявках. Адрес отправителя должен принадлежать подтверждённому в Resend домену.
...
Проверка не выводит секреты и завершает запуск с ошибкой, если ключ Resend, адрес получателя или подтверждённый адрес отправителя не настроены. Сама заявка сначала сохраняется в базе и доступна в `/admin/zayavki/`; почтовое уведомление отправляется после сохранения.
